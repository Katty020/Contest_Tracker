  ...
  useEffect(() => {
    const fetchContests = async () => {
      try {
        ...
        // Filter only CodeChef contests
        const codechefContests = data.filter((contest: Contest) => contest.platform === "codechef")
        ...
        if (bookmarked) {
          ...
          setContests(
            codechefContests.map((contest: Contest) => ({
              ...contest,
              bookmarked: bookmarkedIds.includes(contest.id),
            })),
          )
        } else {
          setContests(codechefContests)
        }
    ...
    <div className="container mx-auto px-4 py-8">
      <header className="flex flex-col md:flex-row justify-between items-center mb-8">
        <div className="flex items-center mb-4 md:mb-0">
          ...
          <h1 className={`text-3xl font-bold ${theme === "dark" ? "text-white" : "text-black"}`}>CodeChef Contests</h1>
        </div>
                ...
                <Card key={contest.id} className="overflow-hidden">
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-300 capitalize">
                        CodeChef
                      </Badge>
                      <Button
                        variant="ghost"
